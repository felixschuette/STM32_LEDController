...
static uint8_t setLEDCmdRxCfg(uint8_t *buf, uint32_t len) {
...
static bool isPacketSizeValid(uint32_t len) {
	...
	if ((len < USB_rxConfig.packet_size)
			&& (USB_rxConfig.packets_cnt + 1 < USB_rxConfig.num_packets)) {
...
void USB_packetRxCallback(uint8_t *buf, uint32_t *len) {
	...
	case receiving:
		...
		if (USB_rxConfig.num_packets == USB_rxConfig.packets_cnt) {
			debug_log(
					"transmission completed, handling packet to application...");
			USB_rxConfig.application_cb(USB_rxConfig.buf,
					USB_rxConfig.buf_length);
			resetRxConfig();
		}
		break;
	}
